  ...
  <div v-if="totalPages" class="pagination">
    <div class="pagination-controls">
      ...
      >
        ...
        <button v-else-if="page === '<'"
          class="page-button arrow"
          @click="emits('update:start', Math.max(0, props.start - props.num))"
        >
          &lt;
        </button>
        <button v-else-if="page === '>'"
          class="page-button arrow"
          @click="emits('update:start', props.start + props.num)"
        >
          &gt;
        </button>
        <button v-else
          class="page-button"
          @click="emits('update:start', (Number(page) - 1) * props.num)"
        >
          {{ page }}
        </button>
      </template>
...
<script setup lang="ts">
  ...
  function paginate(
    ...
    for (let s = Math.min(middleTotal, X); s >= 1; s--) {
      ...
      for (let L2 = a; L2 <= b; L2++) {
        ...
        if (tot <= X) {
          best = { s, L: L2, R: R2, leftEll: le, rightEll: re };
          break;
        }
      }
      if (best) {
    ...
    if (!best) {
      ...
      if (arrows + 2 + ell <= X) {
        best = { s: 0, leftEll: total > 2, rightEll: false };
      }
    }
    ...
    if (!best) {
      ...
      for (let p = best.L ?? 0; p <= (best.R ?? 0); p++) {
        out.push(p);
      }
      if (best.rightEll) {
        out.push("...");
  ...
  const pagesToShow = computed(() => {
    const pages = paginate(totalPages.value, currentPage.value, !props.known);
    return pages;
  });
