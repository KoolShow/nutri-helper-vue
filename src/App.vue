<template>
  <div>
    ...
    <ResultList v-if="searching" :result-items="resultItems" :result-html="resultHtml" :end="isEnd"/>
...
watch(() => pagination.value.current, () => onSubmit(true));
watch(() => pagination.value.per_page, () => onSubmit(true));
watch(searchValue, () => {searchValueChanged.value = true;});
...
async function onSubmit(isPopState = false) {
  ...
  if (searchValue.value) {
    ...
    if (end) {
      ...
      pagination.value.total = Math.max(pagination.value.total, pagination.value.current + resultItems.value.length) ;
    }
